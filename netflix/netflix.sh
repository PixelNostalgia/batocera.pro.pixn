...
# --------------------------------------------------------------------
# --------------------------------------------------------------------
APPNAME=NETFLIX # for installer info
...
# --------------------------------------------------------------------
# -- prepare Ports file, 
port=/userdata/system/pro/$appname/Netflix.sh
rm -rf $port 2>/dev/null
echo '#!/bin/bash ' >> $port
echo 'unclutter-remote -s' >> $port
echo 'DISPLAY=:0.0 LD_LIBRARY_PATH=/userdata/system/pro/.dep:$LD_LIBRARY_PATH /userdata/system/pro/'$appname'/'$AppName'.AppImage --no-sandbox --test-type --start-fullscreen --force-device-scale-factor=1.6 "netflix.com"' >> $port
dos2unix $port 
chmod a+x $port 
...
if [[ ! -e "/userdata/roms/ports/Netflix.sh.keys" ]]; then
  ...
  wget -q -O "/userdata/roms/ports/Netflix.sh.keys" "https://raw.githubusercontent.com/PixelNostalgia/batocera.pro.pixn/main/netflix/extra/Netflix.sh.keys"
