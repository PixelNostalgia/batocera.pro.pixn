...
#      DEFINE APP INFO:
APPNAME=7zip 
APPLINK=https://github.com/PixelNostalgia/batocera.pro.pixn/raw/main/7zip/extra/7zip.zip 
...
COMMAND='batocera-wine lutris play /userdata/system/pro/'$appname'/7zFM.exe 2>/dev/null'
# --------------------------------------------------------------------
######################################################################
######################################################################
######################################################################
...
cd $temp
curl --progress-bar --remote-name --location "$APPLINK" > /dev/null
yes "A" | unzip -qq $appname.zip 2>/dev/null
mv $temp/$appname/* $pro/$appname/ 2>/dev/null
