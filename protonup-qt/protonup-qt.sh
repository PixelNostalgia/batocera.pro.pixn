#!/usr/bin/env bash 
######################################################################
# BATOCERA.PRO/PROTONUP-QT INSTALLER
######################################################################
# --------------------------------------------------------------------
APPNAME=PROTONUP-QT # for installer info
appname=protonup-qt # directory name in /userdata/system/pro/...
AppName=$appname # App.AppImage name
APPPATH=/userdata/system/pro/$appname/$appname.AppImage
ORIGIN="https://github.com/DavidoTek/ProtonUp-Qt" # credit & info
# --------------------------------------------------------------------

# Fetch the latest ProtonUp-Qt release download URL from the GitHub API
APPLINK=$(curl -Ls https://api.github.com/repos/DavidoTek/ProtonUp-Qt/releases/latest | grep "browser_download_url.*AppImage" | awk -F '"' '{print $4}' | head -n 1)

# Validate if APPLINK was found
if [ -z "$APPLINK" ]; then
  echo "Failed to retrieve the latest ProtonUp-Qt AppImage link. Exiting..."
  exit 1
fi
