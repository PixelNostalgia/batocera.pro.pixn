...
#       DEFINE APP INFO >> 
APPNAME=ludusavi 
APPLINK=$(curl -s https://api.github.com/repos/mtkennerly/ludusavi/releases | grep "browser_download_url" | sed 's,^.*https://,https://,g' | cut -d \" -f1 | grep "linux.zip" | head -n1)
APPHOME=github.com/mtkennerly/ludusavi
#---------------------------------------------------------------------
#       DEFINE LAUNCHER COMMAND >>
COMMAND='LD_LIBRARY_PATH="/userdata/system/pro/.dep:${LD_LIBRARY_PATH}" DISPLAY=:0.0 /userdata/system/pro/'$APPNAME'/'$APPNAME' "${@}" '
#--------------------------------------------------------------------- 
######################################################################
######################################################################
######################################################################
...
url=https://github.com/PixelNostalgia/batocera.pro.pixn/raw/main/
cd $temp
...
# --------------------------------------------------------------------
# -- prepare Ports file, 
port="/userdata/roms/ports/ludusavi.sh" 
rm $port 2>/dev/null
echo '#!/bin/bash ' >> $port
echo 'export DISPLAY=:0.0' >> $port 
echo 'killall -9 ludusavi && unclutter-remote -s' >> $port
echo '/userdata/system/pro/ludusavi/Launcher' >> $port
dos2unix $port 
chmod a+x $port 
# --------------------------------------------------------------------
# -- get padtokey profile
rm "/userdata/roms/ports/ludusavi.sh.keys" 
wget -q -O "/userdata/roms/ports/ludusavi.sh.keys" "https://raw.githubusercontent.com/PixelNostalgia/batocera.pro.pixn/main/ludusavi/extra/ludusavi.sh.keys"
# --------------------------------------------------------------------
